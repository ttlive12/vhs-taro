...
const CardMulligans: React.FC<CardMulligansProps> = ({ mode, archetype }) => {
  ...
  // 使用useRequest请求数据
  const { data, loading } = useRequest(() => getArchetypeMulligan(mode, archetype), {
      ...
      <View className='cards'>
        <View className='cards-header'>
          ...
          >
            ...
            <Image
              className={`sort-icon ${
                sortType === 'mulliganImpact' ? (sortOrder === 'asc' ? 'asc' : 'desc') : ''
              }`}
              src={sort}
            />
          </View>
          ...
          >
            ...
            <Image
              className={`sort-icon ${
                sortType === 'drawnImpact' ? (sortOrder === 'asc' ? 'asc' : 'desc') : ''
              }`}
              src={sort}
            />
          </View>
          ...
          >
            ...
            <Image
              className={`sort-icon ${
                sortType === 'keptImpact' ? (sortOrder === 'asc' ? 'asc' : 'desc') : ''
              }`}
              src={sort}
            />
          </View>
        ...
        {sortedData.map(item => (
          <View className='cards-item' key={item.id}>
            <View className='cards-item-card' style={{ gridColumn: 1 }}>
              <CardFrame cardId={item.id} cost={item.cost} name={item.name} rarity={item.rarity} />
            </View>
            <Text
              style={{
                gridColumn: 2,
